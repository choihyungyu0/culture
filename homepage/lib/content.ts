...
export const problem = {
  ...
  note:
    "* 17개 시·도 공급 지표(인구·프로그램·밀도)는 공개데이터 실측입니다 — 행안부 주민등록인구(2026-06)와 문화예술교육 프로그램 목록(2025-12)을 운영주소 기준 시·도로 지오코딩(커버리지 ~83%). 제주 6.94 최고 · 울산 1.20 최저 · 전국 평균 2.72 · 최대 5.8배. 교육경험률(Q9)도 2023 국민문화예술활동조사 원시데이터 10,182명 가중 실측입니다(전국 8.5%). 개입 시뮬레이션·분야 구성만 규칙기반 추정입니다.",
...
export const validation = {
  ...
  title: "실측은 실측대로,\n추정에는 '탐색용' 꼬리표를",
  lead:
    "공급·인구·교육경험률은 공개데이터로 실측하고, 규칙기반 시뮬레이션·일부 구성값에만 '탐색용' 배지를 단다. 화려한 예측일수록 검증 부담을 지기 때문이다.",
  methods: [
    {
      no: "01",
      title: "진단 타당성 (지역 상관)",
      desc:
        "17개 시·도에서 공급 밀도와 문화예술교육 경험률(Q9)은 약한 양의 순위상관(Spearman +0.20)으로 방향이 일치한다. 관람률은 밀도와 음의 상관(−0.66)인데, 이는 관람이 교육공급이 아닌 도심 공연장 접근성의 결과이기 때문 — 그래서 경험 축에서 제외했다. 약한 지역 상관은 탐색적이며, 강한 인과 근거는 개인 단위(우측 Key Findings)에 있다.",
    },
    {
      no: "02",
      title: "예측 백테스트",
    ...
    },
    {
      ...
      title: "정직한 표기",
      desc:
        "실측 지표(인구·프로그램·밀도·교육경험률)와 규칙기반 추정(개입 시뮬레이션·분야 구성)을 명확히 구분하고, 추정치에만 '탐색용' 배지를 붙인다. 근거자료 내보내기에도 가정·출처·한계를 함께 담는다.",
    },
  ],
  // 국민문화예술활동조사 마이크로데이터 실증 (제안서 AI 활용 근거)
  findings: {
    kicker: "Key Findings · 국민문화예술활동조사 실증",
    lead: "방법론만이 아니다. 공개데이터로 실제 검증했다.",
    items: [
      {
        stat: "1.20–1.32",
        unit: "오즈비",
        title: "돈보다 접근성",
        desc: "인구·소득·연령·학력을 통제해도 공급 밀도가 참여를 유의하게 높인다. 가구소득은 비유의 — 참여를 가르는 건 돈이 아니라 접근성이다.",
      },
      {
        stat: "2.81",
        unit: "오즈비",
        title: "조기 투자가 평생을 만든다",
        desc: "어린시절 문화예술교육 경험이 성인 참여의 최강 예측요인. 경험 없음 3.6% vs 유·청소년 두 시기 경험 28.4%.",
      },
      {
        stat: "0.73 → 0.87",
        unit: "ROC-AUC",
        title: "참여 예측 모델",
        desc: "8종 분류모델을 5-fold 교차검증으로 벤치마크 → GradientBoosting 최적. 정책 관련 파생변수로 0.87까지 상승.",
      },
    ],
    note: "5-fold 교차검증·부트스트랩 CI. 공급 밀도는 지역 단위·부분적 내생성이 있어 인과가 아닌 연관으로 해석한다(탐색적).",
  },
} as const;
...
export const ask = {
  ...
  appliedPrefix: "트윈 반영",
  disclaimer:
    "AI 응답은 공개데이터 실측(인구·프로그램·밀도·교육경험률)에 근거합니다. 개입 시뮬레이션 부분만 규칙기반 탐색 추정입니다.",
  errorMessage:
